...
const Layout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const handleRoleChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    database.setUserRole(e.target.value as UserRole);
  };
    ...
    <div className="min-h-screen flex bg-gray-50 font-sans">
      ...
      <aside className="hidden md:flex flex-col w-64 bg-[#0a1d37] p-6 fixed h-full z-20 shadow-2xl">
        ...
        <nav className="flex-1 space-y-2">
          ...
          <SidebarLink to="/cadastro" icon="📝" label="Cadastro" active={location.pathname === '/cadastro'} />
          <SidebarLink to="/atletas" icon="👥" label="Atletas" active={location.pathname === '/atletas'} />
          {isAdmin && (
            <SidebarLink to="/configuracoes" icon="⚙️" label="Ajustes" active={location.pathname === '/configuracoes'} />
          )}
        </nav>

        <div className="mt-auto pt-6 border-t border-white/10 space-y-4">
          <div className="px-2">
             <label className="text-[9px] font-black text-[#c5a059] uppercase tracking-widest block mb-1">Acesso (DEMO)</label>
             <select 
               value={currentUser.role}
               onChange={handleRoleChange}
               className="w-full bg-white/5 border border-white/10 text-white text-[10px] font-bold p-2 rounded outline-none focus:border-[#c5a059]"
             >
               <option value="ADMIN" className="bg-[#0a1d37]">ADMINISTRADOR</option>
               <option value="TECNICO" className="bg-[#0a1d37]">TÉCNICO</option>
               <option value="VISUALIZADOR" className="bg-[#0a1d37]">VISUALIZADOR</option>
             </select>
          </div>
      ...
      <main className="flex-1 md:ml-64 relative">
        ...
        <div className="p-4 md:p-10 max-w-7xl mx-auto">
          {children}
          <p className="mt-8 text-center text-[10px] font-bold uppercase tracking-[0.15em] text-gray-400">
            Desenvolvido por 3Brasil
          </p>
        </div>
      </main>
    </div>
